...
% laser detuning normalized by optical decay rate
Delta_tildes = linspace(-2, 2, 1001) * gamma_o;

% power of the control laser
P_lcs = [0.25, 1.0, 2.25] * P_cr;

% detuning corresponding to multiple values of occupancy
Delta_tilde_multis = zeros(length(P_lcs), length(Delta_tildes));
% multiple values of mean optical occupancy
N_o_multis = zeros(length(P_lcs), length(Delta_tildes));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%                  MAIN LOOP                 %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% iterate detunings
for i = 1:length(P_lcs)
    % amplitude of the control laser
    eta_lc = sqrt(mu * gamma_o * P_lcs(i) / hbar / omega_lc);
    % counter 
    idx = 1;

    % iterate input powers
    for j = 1:length(Delta_tildes)
        % coefficients of the cubic
        coeff_0 = 4 * C^2;
        coeff_1 = 8 * C * Delta_tildes(j);
        coeff_2 = 4 * Delta_tildes(j)^2 + gamma_o^2;
        ...
        for k = 1:length(N_os)
            ...
            if imag(N_os(k)) == 0.0
                % update lists
                Delta_tilde_multis(i, idx) = Delta_tildes(j);
...
% plot
hold on
scatter(Delta_tilde_multis(1, :) / gamma_o, N_o_multis(1, :), 1, 'b', 'filled')
scatter(Delta_tilde_multis(2, :) / gamma_o, N_o_multis(2, :), 1, 'g', 'filled')
scatter(Delta_tilde_multis(3, :) / gamma_o, N_o_multis(3, :), 1, 'r', 'filled')
hold off
% x-axis properties
xlabel('$\tilde{\Delta} / \gamma_{o}$', 'Interpreter', 'latex')
% y-axis properties
ylabel('$N_{o}$', 'Interpreter', 'latex')
% legend
legend('$P_{lc} = 0.25 P_{cr}$', '$P_{lc} = P_{cr}$', '$P_{lc} = 2.25 P_{cr}$', 'Interpreter', 'latex')