...
Delta_tildes = linspace(-2, 2, 201) * gamma_o;

% power of the control laser
P_lcs = linspace(0, 5e-15, 201);

% R.-H. criterion count
Z = zeros(length(P_lcs), length(Delta_tildes));
    ...
    for j = 1:length(Delta_tildes)
        ...
        for k = 1:length(N_os)
            ...
            if imag(N_os(k)) == 0.0
                ...
                % effective detuning
                Delta = Delta_tildes(j) + C * real(N_os(k));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% axes
[X, Y] = meshgrid(Delta_tildes / gamma_o, P_lcs / 1e-15);
% colormesh
pcolor(X, Y, Z);
% x-axis properties
xlabel('$\tilde{\Delta} / \gamma_{o}$', 'Interpreter', 'latex');
% y-axis properties
ylabel('$P_{lc}$ (fW)', 'Interpreter', 'latex');
% cbar properties
